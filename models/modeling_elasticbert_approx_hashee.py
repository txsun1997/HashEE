...
class ElasticBertModel(ElasticBertPreTrainedModel):
    ...
    def update_mask(self, hidden_mask, remain_layers):
        ...
        hidden_mask = remain_layers.le(0).view(batch_size, seq_len, 1).expand_as(hidden_mask).float()
    ...
    def forward(
        ...
        extended_attention_mask = get_extended_attention_mask(attention_mask, embedding_output.dtype)

        hidden, pooled_output = None, None
        prev_hidden = embedding_output
        hidden_mask = torch.ones(batch_size, seq_length, self.config.hidden_size)
        hidden_mask, remain_layers = self.update_mask(hidden_mask, exit_layers)  # mask the padding tokens
        for i in range(self.num_hidden_layers):
            ...
            prev_hidden = prev_hidden * hidden_mask + hidden * (1 - hidden_mask)
            hidden_mask, remain_layers = self.update_mask(hidden_mask, remain_layers)
